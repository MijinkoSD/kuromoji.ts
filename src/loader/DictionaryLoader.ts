...
class DictionaryLoader {
  ...
  loadArrayBuffer(
    file: string,
    callback: NodeDictionaryLoaderOnLoad,
  ) {
   ...
   */
  async load(
    load_callback: DictionaryLoaderOnLoad,
  ) {
    const dic = this.dic;
    ...
    const trie = async (): Promise<void> => {
      const whenErr = (err: Error | null, buffers?: Uint8Array) => {
    ...
    await Promise.all([trie, takeDictionaryInfo]);
