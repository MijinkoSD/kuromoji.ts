...
describe("Tokenizer static method test", () => {
  ...
  it("splitByPunctuation", () => {
    expect(Tokenizer.splitByPunctuation("、")).to.deep.equal(["、"]);
  });
  it("splitByPunctuation", () => {
    expect(Tokenizer.splitByPunctuation("。")).to.deep.equal(["。"]);
  });
  ...
  it("splitByPunctuation", () => {
    expect(Tokenizer.splitByPunctuation("、𠮷野屋。漢字。")).to.deep.equal([
      "、",
      "𠮷野屋。",
      "漢字。",
    ]);
...
describe("Tokenizer for IPADic", () => {
  ...
  beforeAll(
      ...
      new Promise((resolve) => {
        kuromoji.builder({ dicPath: DIC_DIR }).build((error, _tokenizer) => {
          if (_tokenizer === undefined) {
            resolve();
            throw new TypeError("tokenizer must not be undefined");
          }
    ...
    1 * 60 * 1000 // To timeout after a minute
  );
